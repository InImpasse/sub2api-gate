...
: "${SUB2API_APP_DATABASE_PASSWORD:?SUB2API_APP_DATABASE_PASSWORD is required with --apply}"
if [ "${#SUB2API_APP_DATABASE_PASSWORD}" -lt 24 ]; then
  echo "SUB2API_APP_DATABASE_PASSWORD must contain at least 24 characters" >&2
  exit 1
fi

password_b64="$(printf '%s' "$SUB2API_APP_DATABASE_PASSWORD" | base64 | tr -d '\n')"
{
